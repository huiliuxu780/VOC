...
## feature/taxonomy-node-config-testing

- **用途：** 在标签体系详情页接入节点配置、示例与测试调试的真实 API，替换 P0 占位交互
- **任务类型：** 功能开发
- **关联页面/模块：** `frontend/src/pages/LabelTaxonomyDetailPage.tsx`、`backend` 节点配置/示例/测试接口
- **基于分支：** main
- **主要改动文件：**
  - `backend/app/models/config.py`
  - `backend/app/api/v1/label_taxonomies.py`
  - `backend/app/api/v1/label_nodes.py`
  - `backend/app/api/v1/router.py`
  - `backend/app/schemas/taxonomy.py`
  - `backend/app/schemas/__init__.py`
  - `backend/app/models/__init__.py`
  - `backend/tests/test_label_taxonomies_api.py`
  - `backend/tests/test_label_nodes_api.py`
...
## refactor/search-selection-source-hook

- **用途：** 将标签搜索中的 keyboard/pointer 来源判定抽为复用 hook，减少页面状态管理复杂度
- **任务类型：** 重构
...
- **验证情况：**
  - lint：不适用（本轮未以 lint 作为门禁）
  - tests：`cd frontend && npm run test -- src/pages/LabelManagementPage.dom.test.ts` 通过（1 file, 9 tests）
  - type-check：`cd frontend && npm run build` 已包含 `tsc -b`，通过
  - build：`cd frontend && npm run build` 通过
  - 手工验证：未执行（本轮以自动化验证为主）
...
- **下一步：** 已合并到 `main`；后续可补充“上下方向键切换候选 + Enter 确认”完整键盘导航能力。
